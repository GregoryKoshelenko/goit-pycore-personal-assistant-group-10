...
class Base(DeclarativeBase):
    ...
    def _data_columns(cls) -> list[str]:
        """Return mapped column names excluding the primary key."""
        return [column.name for column in cls.__table__.columns if column.name != "id"]

    def to_dict(self) -> dict[str, Any]:
        """Convert model instance to a dictionary without id."""
    ...
    def from_dict(cls, item_id: int, data: dict[str, Any]) -> Self:
        """Create a model instance from id and partial data mapping."""
...
class ContactModel(Base):
    ...
    birthday: Mapped[Any | None] = mapped_column(DateTime, nullable=True)
